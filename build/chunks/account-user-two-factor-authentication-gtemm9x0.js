...
import {
  ...
  CheckboxLabel,
  CheckboxRoot
} from "./account-user-two-factor-authentication-ksnvs1j1.js";
import {
  PasswordInput
} from "./account-user-two-factor-authentication-5zw78ck9.js";
import {
  Input
...
import {
  __commonJS,
  __toESM
} from "./account-user-two-factor-authentication-d1wjbx81.js";
...
// src/registry/thornberry/components/form.tsx
...
function createReactiveSystem({ update, notify, unwatched }) {
    ...
    top:
      do {
        ...
        if (sub.flags & 16)
          ...
          if (link2.nextSub !== undefined || link2.prevSub !== undefined)
            stack = {
              value: link2,
              prev: stack
            };
  ...
  function shallowPropagate(link2) {
    ...
    } while ((link2 = link2.nextSub) !== undefined);
  }
...
// node_modules/@tanstack/store/dist/store.js
var Store = class {
...
function createStore(valueOrFn, actions) {
  ...
  if (actions)
    return new Store(valueOrFn, actions);
...
var LiteThrottler = class {
  constructor(fn, options) {
    ...
    if (this.options.leading === undefined && this.options.trailing === undefined) {
...
var EventClient = class {
  ...
  startConnectLoop() {
    if (this.#connectIntervalId !== null || this.#connected)
      return;
...
function makePathArray(str) {
  ...
  for (let i = segStart;i <= len; i++) {
    ...
    if (i === len || char === CC_DOT || char === CC_OPEN || char === CC_CLOSE) {
      ...
      } else if (prev !== CC_CLOSE && !(prev === -1 && char === CC_CLOSE) && !(prev === char && (char === CC_DOT || char === CC_OPEN))) {
        result.push("");
      }
      segStart = i + 1;
      allDigits = true;
...
var defaultValidationLogic = (props) => {
  ...
  const onMountValidator = isAsync ? undefined : { fn: props.validators.onMount, cause: "mount" };
  ...
  const onServerValidator = isAsync ? undefined : { fn: () => {
    return;
  }, cause: "server" };
...
function metaHelper(formApi) {
  ...
  function handleArraySwap(field, index, secondIndex) {
    ...
    affectedFields.forEach((fieldKey) => {
      ...
      if (meta1)
        formApi.setFieldMeta(swappedKey, meta1);
      if (meta2)
        formApi.setFieldMeta(fieldKey, meta2);
    });
  }
...
class FormApi {
  constructor(opts) {
    ...
    this.update = (options) => {
      ...
      if (!shouldUpdateValues && !shouldUpdateState)
        return;
      batch(() => {
      ...
      this.baseStore.setState(() => {
        ...
        if (!values) {
          Object.values(this.fieldInfo).forEach((fieldInfo) => {
            if (fieldInfo.instance && fieldInfo.instance.options.defaultValue !== undefined) {
              nextValues = setBy(nextValues, fieldInfo.instance.name, fieldInfo.instance.options.defaultValue);
            }
          });
        }
      ...
      for (const validateObj of validates) {
        ...
        promises.push(new Promise(async (resolve) => {
          ...
          for (const field of fields) {
            if (this.baseStore.state.fieldMetaBase[field] === undefined && !fieldErrorsFromFormValidators?.[field]) {
              continue;
            }
            const fieldMeta = this.getFieldMeta(field) ?? defaultFieldMeta;
            const {
    ...
    this.resetField = (field) => {
      this.baseStore.setState((prev) => {
        ...
        return {
          ...
          values: targetValue !== undefined ? setBy(prev.values, field, targetValue) : prev.values
        };
      });
    };
    ...
    this.parseValuesWithSchema = (schema) => {
      return standardSchemaValidators.validate({ value: this.state.values, validationSource: "form" }, schema);
    };
    this.parseValuesWithSchemaAsync = (schema) => {
      return standardSchemaValidators.validateAsync({ value: this.state.values, validationSource: "form" }, schema);
    };
    this.timeoutIds = {
      validations: {},
      listeners: {},
    ...
    if (opts?.transform) {
      ...
      for (const errKey of Object.keys(baseStoreVal.errorMap)) {
        ...
        if (errKeyMap === undefined || !isGlobalFormValidationError(errKeyMap)) {
          continue;
        }
    ...
    this.fieldMetaDerived = createStore((prevVal) => {
      ...
      for (const fieldName of Object.keys(currBaseStore.fieldMetaBase)) {
        ...
        if (!prevBaseMeta || currBaseMeta.errorMap !== prevBaseMeta.errorMap) {
          fieldErrors = Object.values(currBaseMeta.errorMap ?? {}).filter((val) => val !== undefined);
      ...
      if (!Object.keys(currBaseStore.fieldMetaBase).length)
        return fieldMeta;
      ...
      for (const group of this.formGroupApis) {
        ...
        for (const fieldName in currFieldMeta) {
          if (fieldName === groupName)
            continue;
          if (!isFieldInGroup(groupName, fieldName))
            continue;
    ...
    let prevBaseStoreForStore = undefined;
...
class FieldApi {
  constructor(opts) {
    ...
    this.setValue = (updater, options) => {
      this.form.setFieldValue(this.name, updater, mergeOpts(options, { dontRunListeners: true, dontValidate: true }));
    ...
    this.validateSync = (cause, errorFromForm) => {
      ...
      batch(() => {
        const validateFieldFn = (field, validateObj) => {
          ...
          const fieldLevelError = validateObj.validate ? normalizeError2(field.runValidator({
            validate: validateObj.validate,
            value: {
    ...
    this.validate = (cause, opts2) => {
      ...
      if (!opts2?.skipFormValidation) {
        for (const group of encompassingGroups) {
          if (group.state.meta.submissionAttempts === 0)
            continue;
      ...
      for (const group of encompassingGroups) {
        ...
        groupAsyncResults.push(group.validateAsync(cause, formValidationResultPromise, {
    ...
    this.parseValueWithSchema = (schema) => {
      return standardSchemaValidators.validate({ value: this.state.value, validationSource: "field" }, schema);
    };
    this.parseValueWithSchemaAsync = (schema) => {
      return standardSchemaValidators.validateAsync({ value: this.state.value, validationSource: "field" }, schema);
    };
    this.triggerOnBlurListener = () => {
    ...
    this.store = createStore((prevVal) => {
      ...
      if (!meta.isTouched && value === undefined && this.options.defaultValue !== undefined && !evaluate(value, this.options.defaultValue)) {
...
// node_modules/@tanstack/form-core/dist/esm/FormGroupApi.js
  ...
  constructor(opts) {
    ...
    this.mount = () => {
      ...
      if (onMount) {
        ...
        let groupFieldErrors = undefined;
      ...
      return () => {
        ...
        for (const [key, validationMeta] of Object.entries(this.fieldInfo.validationMetaMap)) {
          validationMeta?.lastAbortController.abort();
          this.fieldInfo.validationMetaMap[key] = undefined;
        }
    ...
    this.distributeFieldErrors = (errorMapKey, fieldErrors) => {
      ...
      for (const fullName of allNames) {
        ...
        if (!fieldMeta && !newFormValidatorError)
          continue;
    ...
    this.validateSync = (cause, errorFromForm, opts2 = {}) => {
      ...
      batch(() => {
        const validateFieldOrGroupFn = (fieldOrGroup, validateObj) => {
          ...
          if (isGroup) {
            const distributedHasErrored = this.distributeFieldErrors(errorMapKey, groupFieldErrors);
...
function remapStandardSchemaResultForGroup(result) {
  if (!result || typeof result !== "object")
    return result;
  if (!("form" in result) && !("fields" in result))
    return result;
...
import { useMemo as useMemo4, createContext, useContext } from "react";
...
function useSelector(source, selector = (s) => s, options) {
  ...
  return import_with_selector.useSyncExternalStoreWithSelector(subscribe, getSnapshot, getSnapshot, selector, compare);
}
// node_modules/@tanstack/react-form/dist/esm/useForm.js
import { useState as useState4, useMemo as useMemo3, useRef } from "react";
...
var Field = ({
  ...
  const jsxToDisplay = useMemo(() => functionalUpdate(children, fieldApi), [children, fieldApi]);
...
function useUUID() {
  return useState2(() => uuid())[0];
}

// node_modules/@tanstack/react-form/dist/esm/useFormId.js
var _React = React3;
var useFormId = React3.version.split(".")[0] === "17" ? useUUID : _React.useId;

// node_modules/@tanstack/react-form/dist/esm/useFormGroup.js
import { jsx as jsx2, Fragment as Fragment2 } from "react/jsx-runtime";
import { useMemo as useMemo2, useState as useState3 } from "react";
"use client";
function useFormGroup(opts) {
  const [prevOptions, setPrevOptions] = useState3(() => ({
    form: opts.form,
    name: opts.name
  }));
  const [formGroupApi, setFormGroupApi] = useState3(() => {
    return new FormGroupApi({
      ...opts
    });
  });
  if (prevOptions.form !== opts.form || prevOptions.name !== opts.name) {
    setFormGroupApi(new FormGroupApi({
      ...opts
    }));
    setPrevOptions({ form: opts.form, name: opts.name });
  }
  ...
  const extendedFieldApi = useMemo2(() => {
    const reactiveFieldApi = {
      ...formGroupApi,
      handleSubmit: (...props) => {
...
var FormGroup = ({
  ...
  const jsxToDisplay = useMemo2(() => functionalUpdate(children, formGroupApi), [children, formGroupApi]);
  return /* @__PURE__ */ jsx2(Fragment2, { children: jsxToDisplay });
};
...
function LocalSubscribe({
  ...
  return /* @__PURE__ */ jsx3(Fragment3, { children: functionalUpdate(children, data) });
}
function useForm(opts) {
  const fallbackFormId = useFormId();
  const [prevFormId, setPrevFormId] = useState4(opts?.formId);
  const [formApi, setFormApi] = useState4(() => {
  ...
  const extendedFormApi = useMemo3(() => {
    const extendedApi = {
      ...formApi,
      handleSubmit: (...props) => {
    ...
    extendedApi.Field = function APIField(props) {
      return /* @__PURE__ */ jsx3(Field, { ...props, form: formApi });
    };
    extendedApi.FormGroup = function APIFormGroup(props) {
      return /* @__PURE__ */ jsx3(FormGroup, { ...props, form: formApi });
    };
    extendedApi.Subscribe = function Subscribe(props) {
      return /* @__PURE__ */ jsx3(LocalSubscribe, {
        form: formApi,
...
function LocalSubscribe2({
  ...
  return /* @__PURE__ */ jsx4(Fragment4, { children: functionalUpdate(children, data) });
}
function useFieldGroup(opts) {
  const [formLensApi] = useState5(() => {
    ...
    extendedApi.AppForm = function AppForm(appFormProps) {
      return /* @__PURE__ */ jsx4(form.AppForm, { ...appFormProps });
    };
    extendedApi.AppField = function AppField(props) {
      return /* @__PURE__ */ jsx4(form.AppField, { ...formLensApi.getFormFieldOptions(props) });
    };
    extendedApi.Field = function Field2(props) {
      return /* @__PURE__ */ jsx4(form.Field, { ...formLensApi.getFormFieldOptions(props) });
    };
    extendedApi.Subscribe = function Subscribe(props) {
      return /* @__PURE__ */ jsx4(LocalSubscribe2, {
        lens: formLensApi,
        selector: props.selector,
        children: props.children
      });
...
function createFormHook({
  ...
  function useAppForm(props) {
    ...
    const AppForm = useMemo4(() => {
      return ({ children }) => {
        return /* @__PURE__ */ jsx5(formContext2.Provider, { value: form, children });
      };
    }, [form]);
    const AppField = useMemo4(() => {
      const AppField2 = ({ children, ...props2 }) => {
        return /* @__PURE__ */ jsx5(form.Field, { ...props2, children: (field) => /* @__PURE__ */ jsx5(fieldContext2.Provider, { value: field, children: children(Object.assign(field, fieldComponents)) }) });
      };
      return AppField2;
    }, [form]);
    const extendedForm = useMemo4(() => {
  ...
  function withFieldGroup({
    ...
    return function Render(innerProps) {
      const fieldGroupProps = useMemo4(() => {
        return {
...
var { fieldContext: fieldContext2, formContext: formContext2, useFieldContext, useFormContext: useFormContext2 } = createFormHookContexts();
var errorMessages = (errors) => errors.map((error) => typeof error === "string" ? error : error?.message ?? "").filter(Boolean);
...
var TextField = ({ label, className, id, ...rest }) => {
  ...
  return /* @__PURE__ */ jsxs("div", {
    ...
    children: [
      label && /* @__PURE__ */ jsx6(FieldLabel, {
        htmlFor: fieldId,
        children: label
      }),
      /* @__PURE__ */ jsx6(Input, {
      ...
      hasError && /* @__PURE__ */ jsx6(FieldErrors, {
  ...
  return /* @__PURE__ */ jsxs("div", {
    ...
    children: [
      ...
      /* @__PURE__ */ jsx6(ark.textarea, {
        ...
        onChange: (event) => field.handleChange(event.target.value),
        "aria-invalid": hasError || undefined,
        className: cn("flex min-h-16 w-full rounded-md border border-input bg-transparent px-3 py-2 text-sm shadow-sm transition-colors placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary disabled:cursor-not-allowed disabled:opacity-50", hasError && "border-destructive focus-visible:ring-destructive", className),
        ...rest
      }),
      hasError && /* @__PURE__ */ jsx6(FieldErrors, {
        errors: field.state.meta.errors
  ...
  return /* @__PURE__ */ jsxs("div", {
    ...
    children: [
      /* @__PURE__ */ jsxs(CheckboxRoot, {
        ...
        children: [
          /* @__PURE__ */ jsx6(CheckboxControl, {
            children: /* @__PURE__ */ jsx6(CheckboxIndicator, {
              children: /* @__PURE__ */ jsx6(Check, {
                className: "size-3.5"
              })
            })
          }),
          label && /* @__PURE__ */ jsx6(CheckboxLabel, {
            children: label
          }),
          /* @__PURE__ */ jsx6(CheckboxHiddenInput, {})
        ]
      }),
      hasError && /* @__PURE__ */ jsx6(FieldErrors, {
        errors: field.state.meta.errors
...
var SelectField = ({ label, items, placeholder }) => {
  const field = useFieldContext();
  const hasError = field.state.meta.isTouched && field.state.meta.errors.length > 0;
  const collection = useMemo5(() => createListCollection({ items }), [items]);
  return /* @__PURE__ */ jsxs("div", {
    className: "flex w-full flex-col gap-1.5",
    children: [
      /* @__PURE__ */ jsxs(Select, {
        ...
        children: [
          label && /* @__PURE__ */ jsx6(SelectLabel, {
            className: "text-foreground",
            children: label
          }),
          /* @__PURE__ */ jsx6(SelectControl, {
            children: /* @__PURE__ */ jsxs(SelectTrigger, {
              className: cn("w-full border border-input bg-transparent", hasError && "border-destructive"),
              children: [
                /* @__PURE__ */ jsx6(SelectValueText, {
                  placeholder: placeholder ?? "Select an option"
                }),
                /* @__PURE__ */ jsx6(SelectIndicator, {
          ...
          /* @__PURE__ */ jsx6(SelectPositioner, {
            children: /* @__PURE__ */ jsx6(SelectContent, {
              children: collection.items.map((item) => /* @__PURE__ */ jsxs(SelectItem, {
                item,
                children: [
                  /* @__PURE__ */ jsx6(SelectItemText, {
                    children: item.label
                  }),
                  /* @__PURE__ */ jsx6(SelectItemIndicator, {})
                ]
              }, item.value))
            })
          }),
          /* @__PURE__ */ jsx6(SelectHiddenSelect, {})
        ]
      }),
      hasError && /* @__PURE__ */ jsx6(FieldErrors, {
        errors: field.state.meta.errors
...
var SubmitButton = ({ label, ...rest }) => {
  const form = useFormContext2();
  return /* @__PURE__ */ jsx6(form.Subscribe, {
    selector: (state) => ({
      canSubmit: state.canSubmit,
      isSubmitting: state.isSubmitting
    }),
    children: ({ canSubmit, isSubmitting }) => /* @__PURE__ */ jsx6(Button, {
      type: "submit",
      disabled: !canSubmit,
      ...rest,
...
export { fieldContext2 as fieldContext, formContext2 as formContext, useFieldContext, useFormContext2 as useFormContext, TextField, PasswordField, TextareaField, CheckboxField, SelectField, SubmitButton, useAppForm, withForm, withFieldGroup };
