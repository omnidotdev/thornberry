...
/**
 * Omni brand mark, inlined so the "Made with ... by Omni" credit always carries
 * it (no per-app OmniLogo import to forget). Inherits `currentColor` and is
 * `aria-hidden` because the adjacent "Omni" text already names it.
 */
const OmniLogo = (props: SVGProps<SVGSVGElement>) => (
  <svg
  ...
  >
    ...
    <path d="m350.16,291.36c-13.92,38.42-46.39,68.99-85.59,80.61-13.5,4.1-27.59,5.99-41.62,5.63,120.83-45.38,122.54-217.31,1.77-263.96,90.07-3.14,158.73,93.67,125.44,177.72Z" />
    ...
    <path d="m175.28,286.51c-24.18-.77-45.05-6.12-62.62-14.81-29.99-16.17-53.17-44.03-64.23-76.15,0,0,0-.02,0-.02-9.94-32.31-7.74-68.25,6.53-98.97,13.75-28.49,38.28-51.08,67.17-63.51,0,0,.02,0,.03-.02,15.92-6.35,34.22-10.1,54.9-10.48-120.82,45.36-122.54,217.31-1.77,263.95Z" />
...
const FooterLink = ({ className, ...rest }: ComponentProps<"a">) => (
  <a
    target="_blank"
    rel="noreferrer noopener"
    className={cn(
      "rounded px-2 py-1 text-sm transition-colors hover:text-foreground",
      className,
    )}
    {...rest}
  />
...
const AppFooter = ({
  ...
  return (
    <footer
      className={cn(
        "flex w-full flex-col items-center justify-center gap-2 p-4 text-muted-foreground sm:flex-row sm:gap-1",
        className,
      )}
      {...rest}
    >
      <p className="flex flex-wrap items-center justify-center gap-x-1.5 gap-y-1 px-3 text-sm">
        {appLogo}
        <span>&copy; {new Date().getFullYear()}</span>
        <span aria-hidden="true">&middot;</span>
        <span className="flex items-center gap-1.5">
          Made with
          <span aria-hidden="true">{appSymbol}</span>
          by
          <FooterLink
            href={orgUrl}
            className="inline-flex items-center gap-1 px-0"
          >
            <OmniLogo className="size-4" />
            {orgName}
          </FooterLink>
        </span>
      </p>
